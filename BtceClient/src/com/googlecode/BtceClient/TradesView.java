...
public class TradesView extends View {
	...
	public TradesView(Context context, AttributeSet attrs) {
		...
		ask_line_color = array.getColor(R.styleable.TradesView_td_asklineColor,
				0XFF4E8CD9);
		price_line_color = array.getColor(R.styleable.TradesView_td_pricelineColor,
				0X00FF00FF);
		ask_line_size = array
...
//		}
	...
	public void onDraw(Canvas canvas) {
		...
		price_path.moveTo((float) (r_chart.left + block_width / 2 + 0 * block_width),
				(float) (m_trades_items.get(size-1).price * price_k + price_b));
		r_block.set(
				(int) (r_chart.left + block_width / 2 + 0 * block_width - amount_line_width / 2),
				(int) (m_trades_items.get(0).amount * amount_k + amount_b),
				(int) (r_chart.left + block_width / 2 + 0 * block_width + amount_line_width / 2),
				r_chart.bottom);
		...
		canvas.drawLine(r_block.centerX(), r_block.top, r_block.centerX(), r_block.bottom,mPaint);
		for (int i = size-2; i >=0; --i) {
			price_path.lineTo((float) (r_chart.left + block_width / 2 + (size-i-1) * block_width),
					(float) (m_trades_items.get(i).price * price_k + price_b));
			r_block.set(
					(int) (r_chart.left + block_width / 2 + (size-i-1) * block_width - amount_line_width / 2),
		...
		if (mousedown) {
			float index =  size -1 - (ex - r_chart.left - block_width / 2)/block_width;
			if(index < size && index >= 0) {
				...
				r_block.set(
						...
						(int) (r_chart.left + block_width / 2 + (size-i-1) * block_width + amount_line_width / 2),
						r_chart.bottom);
				canvas.drawLine(r_block.centerX(), r_chart.top, r_block.centerX(), r_chart.bottom,mPaint);
				mPaint.setStrokeWidth(point_size);
				mPaint.setColor(point_color);
				canvas.drawPoint(r_chart.left + block_width / 2 + (size-i-1) * block_width,
				(float) (m_trades_items.get(i).price * price_k + price_b), mPaint);
				mPaint.setColor(point_color ^ 0x00FFFFFF);
				canvas.drawPoint(r_chart.left + block_width / 2 + (size-i-1) * block_width,
						 (float) (m_trades_items.get(i).amount * amount_k + amount_b), mPaint);
				// draw the info text
				String info = (m_trades_items.get(i).trade_type==0? "Ask:":"Bid:")
						+ CandleStickView.my_formatter(
								m_trades_items.get(i).price, 6)
						+ " / "
						+ CandleStickView.my_formatter(
								m_trades_items.get(i).amount, 8)
						+ " / "
						+ error_time_format.format(m_trades_items.get(i).date * 1000);
